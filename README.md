...
- Change user to portal@airwave.tv, or user who logs in

- Move %API into a config file

Usage reports - Upload the Techlive usage reports all at once, maybe in a .zip file

load-events-airtime

- does not split record correctly if film name has a comma

- does not cater for double quotes delimiting text (to cater for embedded commas)
...
- Special files over 3 months old

All Perl scripts - Port to Windows ActiveState Perl

API.pm

- Change user to prep@airwave.tv
...
API.pm
- Change user to distro@airwave.tv
